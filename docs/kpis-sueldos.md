...
- **Fórmula:** `(aprobadas + aprobadas con observación) ÷ total auditadas × 100`. *(excluye
  `aprobado_automatico` del numerador y del total — `kpiAprobacionPrimera(..., usuarioId=null)` lo
  filtra a nivel de sección. Por trabajador, en cambio, cuenta como aprobada: ver decisión del
  22/09/2026.)*
...
| 22/09/2026 | **«Aprobación a la primera» con el 4° veredicto (reintegración del jefe, v6.5):** el jefe había cambiado `kpiAprobacionPrimera` para contar `aprobado_automatico` como aprobada en TODOS los niveles («decisión de Gerencia 16/09»), y eso chocaba con la ficha: la tarjeta de la sección y «Aprobación a la 1ª (sección)» daban números distintos. Queda la regla del 15/09 aplicada por nivel: **por trabajador** cuenta como aprobada (Gerencia: el trabajador no pierde puntos); **a nivel de sección** queda fuera (S2.3, mide inspecciones reales). |
